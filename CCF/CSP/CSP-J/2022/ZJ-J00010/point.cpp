...
int main(){
	...
	if(k == 0){
		...
		for(int i = 1;i <= n;i++){
			if((cjm[i].x == cjm[i+1].x && cjm[i].y+1 == cjm[i+1].y) || (cjm[i].y == cjm[i+1].y && cjm[i].x+1 == cjm[i+1].x) || (cjm[i].x == cjm[i+1].x && cjm[i].y+k == cjm[i+1].y) || (cjm[i].y == cjm[i+1].y && cjm[i].x+k == cjm[i+1].x)) ans++;
		}
		cout<<ans;
	}
	return 0;
